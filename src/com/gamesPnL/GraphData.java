	...
	protected void onResume() {
		...
		if (result.getCount() > 0) {
			...
			if (result != null)
				result.close();
			/*
			 * String[] verlabels = new String[] { "$" + df.format(max), "$" +
			 * df.format(((min+max)/2)), "$" + df.format(min) }; String[]
			 * horlabels = new String[] { "", "" }; GraphView graphView = new
			 * GraphView(this, values, "Running Total", horlabels, verlabels,
			 * GraphView.LINE); setContentView(graphView);
			 */
			...
			for (int k = 0; k < length; k++) {
				((XYSeriesRenderer) mRenderer.getSeriesRendererAt(k))
						.setFillPoints(true);
			}
			...
			if (mChartView == null) {
				try {
					...
					gamesLogger.i(
							TAG,
							SubTag + "mDataset count: "
									+ mDataset.getSeriesCount());
