...
public class GamePnLTrackerProvider extends ContentProvider {
	...
	private static class DbAdapter extends SQLiteOpenHelper {
			...
			try {
				...
				if (c.getCount() == 0) {
					db.execSQL("CREATE TABLE " + PNL_TABLE_NAME
							...
							+ "evYear TEXT, " + "evMonth TEXT, "
							+ "evDay TEXT, " + "evDate DATE, "
							+ "eventType TEXT, " + "gameType TEXT, "
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			String sql = null;
			Cursor from;
			gamesLogger.i(TAG, SubTag + "starting an upgrade from "
			...
			try {
					...
					do {
						...
						if (!from.getString(2).equals("")
							...
							case 8:
								...
								dayS = String.format("%02d",
										Integer.parseInt(dayS));
								monthS = String.format("%02d",
										Integer.parseInt(monthS));
								yearS = String.format("%04d",
							...
							case 9:
								/*
								 * Format does not change, just adding one more
								 * game
								 */
								break;
							default:
				...
				// copy all the records from the <table>_old to <table>
				sql = "SELECT name,status FROM " + PNL_STATUS_TABLE_NAME
						+ "_OLD;";
				gamesLogger.i(TAG, SubTag + "exec sql: " + sql);
				from = db.rawQuery(sql, null);
				if (from.moveToFirst()) {
			...
			gamesLogger.i(TAG, SubTag
					+ "Backup: dataChanged() after db upgrade");
			bkpMgm.dataChanged();
	...
	public int delete(Uri uri, String where, String[] whereArgs) {
		...
		/* Backup the data */
		gamesLogger
				.i(TAG, SubTag + "Backup: dataChanged() after deleting data");
		bkpMgm.dataChanged();
		return count;
	}
	...
	public Uri insert(Uri uri, ContentValues values) {
		gamesLogger.i(TAG, SubTag + "Inserting data");
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		switch (sURIMatcher.match(uri)) {
		case USER:
		...
		/* Backup the data */
		gamesLogger.i(TAG, SubTag + "Backup: dataChanged() after data insert");
		bkpMgm.dataChanged();
		return null;
	}
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		try {
			...
			// result = qb.query(db, projection, selection, selectionArgs, null,
			// null, null);
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		try {
			...
			switch (sURIMatcher.match(uri)) {
			case USER:
				count = db.update(USER_TABLE_NAME, values, selection,
						selectionArgs);
				gamesLogger.i(TAG, SubTag
						+ "Backup: dataChanged() after user update");
				bkpMgm.dataChanged();
				break;
			case PNLDATA:
				gamesLogger.i(TAG, SubTag + "Updating " + PNL_TABLE_NAME);

				count = db.update(PNL_TABLE_NAME, values, selection,
						selectionArgs);
				gamesLogger.i(TAG, SubTag
						+ "Backup: dataChanged() after result update");
				bkpMgm.dataChanged();
				break;
			case PNLSTATUS:
				count = db.update(PNL_STATUS_TABLE_NAME, values, selection,
						selectionArgs);
				gamesLogger.i(TAG, SubTag
						+ "Backup: dataChanged() after status update");
				bkpMgm.dataChanged();
				break;
			case PNLGAMES:
				count = db.update(PNL_GAMES_TABLE_NAME, values, selection,
						selectionArgs);
				gamesLogger.i(TAG, SubTag
						+ "Backup: dataChanged() after game update");
				bkpMgm.dataChanged();
				break;
			default:
				throw new IllegalArgumentException("Unknown URI " + uri);
